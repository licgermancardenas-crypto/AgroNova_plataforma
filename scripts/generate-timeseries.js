/**
 * AgroNova GIS-12 — province_timeseries.json generator
 * Run: node scripts/generate-timeseries.js
 */
...
const REV_GROWTH  = { PAM:0.280, NOA:0.320, NEA:0.300, CUY:0.250, PAT:0.220 };
const CLI_GROWTH  = { PAM:0.090, NOA:0.115, NEA:0.105, CUY:0.082, PAT:0.070 };
const OTIF_DELTA  = 0.65;
const CHURN_DELTA = 0.015;
const MARGIN_DELTA= 0.12;
const BASELINE    = 2026;
...
function kpiForYear(kpi, year) {
  const yb = BASELINE - year;
  if (yb === 0) return { ...kpi };
  const rr = REV_GROWTH[kpi.macro_region] ?? 0.27;
  const cr = CLI_GROWTH[kpi.macro_region] ?? 0.09;
  const s  = seed(kpi);
  const rf = Math.pow(1 + rr * (1 + s * 0.05), yb);
  const cf = Math.pow(1 + cr * (1 + s * 0.03), yb);
  return {
    ...kpi,
    revenue_ars:  Math.max(1000000, Math.round(kpi.revenue_ars / rf)),
    revenue_pct:  0,
    n_activos:    Math.max(1, Math.round(kpi.n_activos / cf)),
    n_clientes:   Math.max(1, Math.round(kpi.n_clientes / Math.pow(1 + cr * 0.8 * (1 + s * 0.02), yb))),
    margen_pct:   parseFloat(Math.max(12, kpi.margen_pct - MARGIN_DELTA * yb + s * 0.3).toFixed(1)),
    otif_pct:     parseFloat(Math.max(70, kpi.otif_pct - OTIF_DELTA * yb + s * 0.2).toFixed(1)),
    churn_score:  parseFloat(Math.min(0.90, kpi.churn_score + CHURN_DELTA * yb + Math.abs(s) * 0.01).toFixed(3)),
  };
}

const result = {};
for (let year = 2016; year <= 2026; year++) {
  const raw   = PROVINCE_KPIS.map(k => kpiForYear(k, year));
  const total = raw.reduce((s, k) => s + k.revenue_ars, 0);
  result[year] = raw.map(k => ({
    ...k,
