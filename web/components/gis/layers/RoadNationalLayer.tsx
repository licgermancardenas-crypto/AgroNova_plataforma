...
const STYLE = TRANSPORT_STYLE.nacional;
const PANE  = "roadNationalPane";

function RoadNationalLayer({ visible }: Props) {
  const map = useMap();
  const [data, setData] = useState<GeoJSON.FeatureCollection | null>(null);
  const layerRef = useRef<L.GeoJSON | null>(null);
  const fetchedRef = useRef(false);

  // lazy fetch — only once the layer is first switched on
  useEffect(() => {
    if (!visible || fetchedRef.current) return;
    fetchedRef.current = true;
    fetch("/data/geo/transport/road_national.geojson")
      .then(r => r.json())
      .then(setData)
  ...
  useEffect(() => {
    ...
    const gj = L.geoJSON(data, {
      ...
      onEachFeature: (feature, layer) => {
        ...
        layer.bindTooltip(
          roadTooltip(p.nombre, "Ruta Nacional", p.longitud_km, p.provincia, STYLE.color),
          { sticky: true, direction: "top" },
        );
        layer.on({
          mouseover: (e) => (e.target as L.Path).setStyle({ weight: STYLE.weight + 1.4, opacity: 1 }),
...
export default memo(RoadNationalLayer);
